...
def new_weights(shape, name="weight"):
    ...
    return tf.Variable(tf.compat.v1.truncated_normal(shape, stddev=0.1), name=name)
...
class QNet():
    def __init__(self, trainable=False, name="DQN"):
        ...
        self.env_obs = tf.compat.v1.placeholder(shape=[None, env_features], dtype=tf.float32)
        ...
        if trainable:
            self.q_next = tf.compat.v1.placeholder(shape=[None, env.action_space.n], dtype=tf.float32, name="{}_q_next".format(name))
            self.loss = tf.reduce_mean(tf.compat.v1.squared_difference(self.q_next, self.q_out))
            tf.summary.scalar('loss_function', self.loss)
            self.train_op = tf.compat.v1.train.RMSPropOptimizer(0.05).minimize(self.loss)
...
def main():
    ...
    with tf.compat.v1.Session() as sess:
        ...
        # writer = tf.compat.v1.summary.FileWriter("train_summary", sess.graph)
        f_reward = 0
